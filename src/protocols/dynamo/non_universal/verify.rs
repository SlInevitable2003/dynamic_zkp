//! Dynamo(非通用版)Verify(README §1.1.3)。
...
/// 验证证明。
///
/// 注意:论文里 `G_T` 用加法表示,Verify 的 `+` 在 arkworks 的 `Fq12`(乘法表示)中对应 `*`。
pub fn verify(vk: &VerifyingKey, x: &Instance, pi: &Proof) -> bool {
    let g2 = G2::generator();
    let xm1 = vk.xm - g2; // G2 上的 τ_X^m - 1
    let ym1 = vk.ym - g2; // G2 上的 τ_Y^m - 1
    ...
    // e(z_zk, Ym) == e(Z_zk, g2)
    if pair(&x.z, &vk.ym) != pair(&pi.z, &g2) {
        return false;
    }

    // e(h_zk, Xm) == e(H_zk, g2)
    if pair(&x.h, &vk.xm) != pair(&pi.h, &g2) {
        return false;
    }

    // e(β_zk, X2) == e(B_zk, g2)
    if pair(&pi.beta, &vk.x2) != pair(&pi.b, &g2) {
        return false;
    }
